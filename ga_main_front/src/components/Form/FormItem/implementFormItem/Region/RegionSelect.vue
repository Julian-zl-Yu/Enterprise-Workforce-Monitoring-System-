<template>
  ...
  >
    ...
    >
      ...
      >
        <!--筛选的时候不显示-->
        <div class="regin-select">
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item
              v-for="(breadcrumb,index) in breadcrumbs"
              :key="breadcrumb.id"
              @click.native="clickBreadcrumb({node:breadcrumb,index,isB:true})"
            >
              {{ index===breadcrumbs.length-1?'*':'' }}{{ breadcrumb.name }}
            </el-breadcrumb-item>
          </el-breadcrumb>
          <el-scrollbar
            ref="suggestionPanel"
            tag="ul"
            class="el-cascader__suggestion-panel region-item-wrapper"
            view-class="el-cascader__suggestion-list"
          >
            <li
              v-for="r in regionList"
              :key="r.id"
              :class="['region-item',{elevation2:r.value===checkedValue}]"
              @click="clickBreadcrumb({node:r})"
            >
              {{ r.name }}
            </li>
          </el-scrollbar>
...
export default {
  ...
  mounted() {
    ...
    /* this.$on("visible-change", (isShow) => {
         if (isShow) {
             const {popper} = this.$refs;
             this.componentWidth = $(reference).width();
  ...
  methods: {
    ...
    setPresentText(presentText, presentTextArray) {
      this.presentText = presentText;
      if (presentTextArray && presentTextArray.length > 0) {
        this.presentTextArray = presentTextArray;
        this.setBreadcrumbs(presentTextArray);
      } else {
        this.presentTextArray = [];
      }
    },
    async clickBreadcrumb({ node, index, isB }) {
      /*如果是Breadcrumbs，但是是最后一节，不跳转*/
      if (isB && index === this.breadcrumbs.length - 1) return;
      const presentTextArray = await getAreaCodeNodeFromDB(node.id);
      this.setBreadcrumbs(presentTextArray);
      // this.setRegionList(node);
    },
    async setBreadcrumbs(presentTextArray) {
      const lastIndex =
        presentTextArray.length > 2 ? presentTextArray.length - 1 : 1;
      const rootNode = await getAreaCodeNodeById(this.regionPid);
      /*比较level即可*/
      const breadcrumbs = presentTextArray
        .slice(0, lastIndex)
        .filter((b) => b.treeLevel > rootNode.treeLevel)
        .sort((i, ii) => i.treeLevel < ii.treeLevel);
      breadcrumbs.unshift(rootNode);
      const shouldUpdateBreadcrumbs = await this.setRegionList(
        last(breadcrumbs)
      );
    ...
    async setRegionList(node) {
      if (!node || !node.id) return;
      const result = await getAreaCodeNodeByPid(node.id);
      /**/
    ...
    handleClear() {
      this.setPresentText("");
      /*TODO:this.panel.clearCheckedNodes();*/
    },
    ...
    computePresentContent() {
      ...
      this.$nextTick(() => {
        if (this.config.multiple) {
          ...
          this.setPresentText(this.presentTags.length ? " " : null);
        } else {
    ...
    computePresentText() {
      ...
      if (!isEmpty(checkedValue)) {
        try {
          (async () => {
            const result = await getAreaCodeNodeFromDB(checkedValue);
            if (isArray(result)) {
              this.setPresentText(
                result.map((i) => i.name).join(this.separator),
                result
              );
            }
          })();
...
.regin-select {
  padding: 12px;
  .el-cascader__suggestion-panel.region-item-wrapper {
    float: left;
    ul.el-scrollbar__view.el-cascader__suggestion-list {
      text-align: left;
      .region-item {
        margin: 6px;
        padding: 2px 4px;
        ...
        &.elevation2 {
          color: $color-btn-add;
          border: 1px solid $color-btn-add;
          font-weight: 700;
        }
        &:hover {
          color: $color-btn-add;
