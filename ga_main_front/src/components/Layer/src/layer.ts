...
let Notification = (function (Vue, globalOption = {
  ...
  self.open = function (options) {
    ...
    let id = options.type === 4 ? options.title.substring(1) + "-tips" : `notification_${new Date().getTime()}_${seed++}`;
    options.id = id;
    options.layer = self;
   ...
   * 手动最大化
   */
  self.full = (id = "") => document.querySelector("#" + id + " .lv-icon-max").click();
  /**
   * 手动最小化
   */
  self.min = (id = "") => document.querySelector("#" + id + " .lv-icon-mini").click();
  /**
   * 手动最小化
   */
  self.restore = (id = "") => document.querySelector("#" + id + " .lv-icon-huanyuan").click();
