<template>
  <div>
    ...
    <el-button 
      ...
      @click="rejectTask()"
    >
      {{ $t('process.rejectTask') }}
    </el-button>
    <el-button 
      type="success" 
      @click="rollbackTask()"
    >
      {{ $t('process.doBackRollback') }}
    </el-button>
    <el-button 
      type="info" 
      @click="entrustTask()"
    ...
    <el-button 
      type="danger" 
      @click="terminationTask()"
    >
      {{ $t('process.terminationTask') }}
    </el-button>
    <ren-task-back 
      v-if="renTaskBackVisible" 
      ref="renTaskBack"
    />
    <ren-task-entrust 
      v-if="renTaskEntrustVisible" 
      ref="renTaskEntrust"
    />
    <ren-task-handle 
      v-if="renTaskHandleVisible" 
      ref="renTaskHandle"
    />
  </div>
</template>
