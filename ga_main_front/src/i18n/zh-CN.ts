...
(function checkBtnTypeChineseName() {
    let log = "";
    forEach(BTN_TYPE, (typeItem) => {
        if (isString(typeItem)) {
            if (!t.btnType[typeItem]) {
                log += `BTN_TYPE[${typeItem}] 未设置对应中文名;\n`;
            }
        }
    });
...
t.loading = "加载中...";
t.brand = {};
t.brand.lg = "广安市企业用工监测系统";
t.brand.mini = "金键";

t[BTN_TYPE[BTN_TYPE.generate]] = "生成";
t.add = "新增";
t.reset = "重置";
t.delete = "删除";
t.deleteBatch = "删除";
...
t.home.sysInfo.versionVal = SITE_CONFIG["version"];
