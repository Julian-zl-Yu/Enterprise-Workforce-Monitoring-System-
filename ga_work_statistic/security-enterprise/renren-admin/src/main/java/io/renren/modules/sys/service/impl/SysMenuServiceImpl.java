...
public class SysMenuServiceImpl extends BaseServiceImpl<SysMenuDao, SysMenuEntity> implements SysMenuService {
	...
	@Override
	public List<SysMenuDTO> getUserRoleMenuList(Long roleId, int type) {
		List<SysMenuEntity> menuList;
		UserDetail user = SecurityUser.getUser();
		//系统管理员，拥有最高权限
		if (user.getSuperAdmin() == SuperAdminEnum.YES.value()) {
			menuList = baseDao.getMenuList(type, HttpContextUtils.getLanguage());
			menuList = baseDao.getAdminMenuList(type, HttpContextUtils.getLanguage());
		} else {
			menuList = baseDao.getUserRoleMenuList(roleId, user.getId(), type, HttpContextUtils.getLanguage());
		}
